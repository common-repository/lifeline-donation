...
function wpmc_posts_array($field_args) {
	...
	$options = array('' => esc_html__( '--Select--', 'lifeline-donation' ));
...
function wpcm_sidebar_data($args) {
	...
	$data = array('' => esc_html__('No Sidebar', 'lifeline-donation'));
...
function wpcm_get_success_page_url($args) {
	...
	if( ! $page_id ) {
		if( defined('DOING_AJAX' ) ) {
			wp_send_json_error( array('message' => esc_html__( 'Unable to find success page, Plese contact administrator', 'lifeline-donation' )) );
		} else {
			throw new Exception(esc_html__( 'Unable to find success page, Plese contact administrator', 'lifeline-donation' ), 1);
	...
	if( ! esc_url( $url ) ) {
		if( defined('DOING_AJAX' ) ) {
			wp_send_json_error( array('message' => esc_html__( 'Invalided success page URL, Plese contact administrator', 'lifeline-donation' )) );
		} else {
			throw new Exception(esc_html__( 'Invalid success page URL, Plese contact administrator', 'lifeline-donation' ), 1);
		}
	}
 ...
 * @return [type]        [description]
 */
function webinane_cm_price_with_symbol($price) {
	$settings = wpcm_get_settings();
 ...
 * @return [type] [description]
 */
function webinane_currency_symbol() {
...
if( ! function_exists('wpcm_validate_nonce') ) {
	function wpcm_validate_nonce($nonce = null, $ajax = true) {
		...
		if(! $valid) {
			$message = esc_html__('Security check is failed, please refresh the page and try again', 'lifeline-donation');
			if($ajax) {
...
if ( ! function_exists('wpcm_customer_billing_fields') ) {
	...
	function wpcm_customer_billing_fields() {
		...
		$keys = ['first_name', 'last_name', 'address_line_1', 'city', 'base_country', 'state', 'zip', 'phone'];
