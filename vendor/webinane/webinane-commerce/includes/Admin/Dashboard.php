...
class Dashboard {
	...
	private function is_authorized() {
		...
		if ( ! $authorized ) {
			wp_send_json_error( array( 'message' => esc_html__( 'You are not authorized to do this action', 'lifeline-donation' ) ) );
		}
	...
	public function customers() {
		...
		wp_send_json_success( $customers->toArray() );
	}
	...
	public function update_customer() {
		...
		if ( ! array_get( $form, 'id' ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Invalid Customer ID provided', 'lifeline-donation' ) ) );
		}

		$customer = Customer::find( array_get( $form, 'id' ) );
		if ( ! $customer ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Specified customer is not found', 'lifeline-donation' ) ) );
		}
		$email = sanitize_email( array_get( $form, 'email' ) );
		if ( ! $email ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Invalid customer email provided', 'lifeline-donation' ) ) );
		}
		...
		wp_send_json_success( array( 'message' => esc_html__( 'Customer is updated Successfully', 'lifeline-donation' ) ) );
	...
	public function remove_customer() {
		...
		if ( ! $customer ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Customer is not found', 'lifeline-donation' ) ) );
		}

		$customer->delete();
		wp_send_json_success( array( 'message' => esc_html__( 'Customer is deleted Successfully', 'lifeline-donation' ) ) );
	}
