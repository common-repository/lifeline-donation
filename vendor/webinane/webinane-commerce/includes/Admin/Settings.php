...
class Settings {
		...
		add_filter('display_post_states', function( $states, $post ) {
		    ...
		    if ( 'page' == get_post_type( $post->ID ) ) {
		    	if($post->ID == $settings->get('checkout_page')) {
		            $states[] = __('WPCM Checkout Page', 'lifeline-donation');
		    	} else if($post->ID == $settings->get('success_page')) {
		            $states[] = __('WPCM Order Success Page', 'lifeline-donation');
		    	} else if($post->ID == $settings->get('my_account_page')) {
		            $states[] = __('WPCM My Account Page', 'lifeline-donation');
		    	}
		    }
		    return $states;
		}, 10, 2);
	...
	static function ajax() {
		...
		if(! wp_verify_nonce( $nonce, 'wp_rest' )) {
			wp_send_json_error( ['message' => esc_html__('Sucurity verification failed', 'lifeline-donation')], 403 );
		}
		...
		if( $subaction == 'fields_data') {
			...
			if ( $data ) {
				...
				wp_send_json( array('options' => $options, 'message' => esc_html__('Settings are updated successfully', 'lifeline-donation' ) ) );
			} else {
				wp_send_json( array('message' => esc_html__('Something went wrong', 'lifeline-donation' )), 403 );
			}
			...
			switch (array_get($_post, 'input_type')) {
				case 3:
					...
					if(! array_get($file, 'tmp_name') || array_get($file, 'type') !== 'application/json') {
						wp_send_json_error( esc_html__('Invalid File provided', 'lifeline-donation' ) );
					}
				...
				case 6:
					if(! esc_url($import_input)) {
						wp_send_json_error( esc_html__('Invalid URL Provided', 'lifeline-donation' ) );
					}
			...
			if( ! $text ) {
				wp_send_json_error( esc_html__('Please provide a proper json data', 'lifeline-donation' ) );
			}
			if( is_array($text) ) {
				update_option(WPCM_SETTINGS_KEY, $text);
				wp_send_json_success( esc_html__('Settings are imported successfully', 'lifeline-donation' ) );
			}
			exit;
		}
		exit;
	...
	static function register() {
		self::$menu = add_menu_page(
	        apply_filters( 'webinane_commerce/settings/page_heading', esc_html__( 'WP Commerce Settings', 'lifeline-donation' ) ),
	        apply_filters( 'webinane_commerce/settings/menu_label', esc_html__( 'WP Commerce', 'lifeline-donation' ) ),
	        'manage_options',
		...
		Dashboard::$dashboard = add_submenu_page(
	        'wp-commerce-settings',
	        esc_html__( 'Dashboard', 'lifeline-donation' ),
	        esc_html__('Dashboard', 'lifeline-donation' ),
	        'manage_options',
		...
		self::$connect_menu = add_submenu_page(
			'wp-commerce-settings',
	        esc_html__( 'Webinane Extensions', 'lifeline-donation' ),
	        esc_html__('Extensions', 'lifeline-donation' ),
	        'manage_options',
	...
	static function upload_image() {
		...
		if( ! is_user_logged_in() ) {
			wp_send_json_error(['message' => esc_html__('You are not authorized', 'lifeline-donation')]);
		}
		...
		if ( $movefile && ! isset( $movefile['error'] ) ) {
		    wp_send_json_success(['message' => esc_html__('File is valid, and was successfully uploaded.', 'lifeline-donation'), 'url' => $movefile['url']]);
		} else {
		    /*
