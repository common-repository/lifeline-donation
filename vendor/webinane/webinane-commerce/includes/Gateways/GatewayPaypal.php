...
class GatewayPaypal extends Gateways
{
	...
	static function setup() {
		$gateways = array_get(wpcm_get_settings(), 'gateways.paypal_express_gateway');
		$gateways = ($gateways) ? $gateways : wpcm_get_settings();

		self::instance()->name = esc_html__( 'PayPal Standard', 'lifeline-donation' );
		self::instance()->desc = esc_html__( 'Pay using PayPal', 'lifeline-donation' );
	...
	static function settings($settings = array()) {
		...
		$settings[] = array(
			'title'			=> esc_html__( 'PayPal Standard', 'lifeline-donation' ),
			'icon'			=> 'fa fa-th',
			'id'			=> 'paypal_express_gateway',
			'heading'	=> esc_html__('PayPal Standard', 'lifeline-donation'),
			"sandbox_help" => esc_html__('Enable sandbox mode to do test payments', 'lifeline-donation'),
			"has_sandbox"	=> true,
			'fields'		=> array(
				Text::make(esc_html__( 'Title', 'lifeline-donation' ), 'paypal_title', function() use ($paypal_gateway) {
				...
				Text::make(esc_html__( 'Description', 'lifeline-donation' ), 'paypal_description', function() use ($paypal_gateway) {
					if($value = array_get(wpcm_get_settings(), 'paypal_description')) {
						return $value;
					}
					return array_get($paypal_gateway, 'paypal_description');
				})->setHelp(esc_html__( 'Description to show on the payment page', 'lifeline-donation' )),
				Text::make(esc_html__( 'PayPal Email', 'lifeline-donation' ), 'paypal_email', function() use ($paypal_gateway) {
					if($value = array_get(wpcm_get_settings(), 'paypal_email')) {
						return $value;
					}
					return array_get($paypal_gateway, 'paypal_email');
				})->setHelp(esc_html__( 'Please enter your PayPal email, it is need to take the orders', 'lifeline-donation' )),

				// API Username
				Text::make(esc_html__( 'API Username', 'lifeline-donation' ), 'paypal_api_username', function() use ($paypal_gateway) {
					if($value = array_get(wpcm_get_settings(), 'paypal_api_username')) {
						return $value;
					}
					return array_get($paypal_gateway, 'paypal_api_username');
				})->setHelp(__('Please enter your PayPal live API username, Learn how to access <a href="https://developer.paypal.com/webapps/developer/docs/classic/api/apiCredentials/#create-an-api-signature" target="_blank">PayPal API Credentials</a>', 'lifeline-donation')),
				// API Password
				Text::make(esc_html__( 'API Password', 'lifeline-donation' ), 'paypal_api_password', function() use ($paypal_gateway) {
					if($value = array_get(wpcm_get_settings(), 'paypal_api_password')) {
						return $value;
					}
					return array_get($paypal_gateway, 'paypal_api_password');
				})->setHelp(esc_html__('Please enter your PayPal live API password', 'lifeline-donation')),

				// Api Signature
				Text::make(esc_html__( 'API Signature', 'lifeline-donation' ), 'paypal_api_signature', function() use ($paypal_gateway) {
					if($value = array_get(wpcm_get_settings(), 'paypal_api_signature')) {
						return $value;
					}
					return array_get($paypal_gateway, 'paypal_api_signature');
				})->setHelp(sprintf(esc_html__( 'Please enter your PayPal live API signature, the IPN listening URL should be %s', 'lifeline-donation' ), $url ) ),
		...
		if(isset($setECResponse)) {
			if($setECResponse->Ack =='Success') {
				...
				if( $setECResponse->Ack == 'Failure') {
					wp_send_json_error( ['message' => $setECResponse->Errors[0]->LongMessage] );
				} else {
					wp_send_json_error( ['message' => esc_html__('Something went wrong', 'lifeline-donation') ] );
				}
			}
		}
		exit;
	...
	static function capture_payment($order, $key) {
		...
		if( $return_type !== 'success' ) {
			echo '<div class="alert alert-danger">' . esc_html__( 'There is something went wrong', 'lifeline-donation' ) . '</div>';
			return;
		}
		...
		if( isset( $detail->PaymentInfo ) ) {
			...
			if( isset( $order->ID ) ) {
				...
				if( $payment_info->PaymentStatus == 'Completed' ) {
					...
					$post_id = wp_update_post( (array) $order, true );
	...
	static function getPeriod($data) {
		...
		$period = webinane_set($data, 'billing_period', 'monthly');
	...
	static function capture_recurring_payment($order, $proflie_id) {
		...
		$total = get_post_meta($order->ID, '_wpcm_order_total', true);
		if( ! $proflie_id ) {
			echo '<div class="alert alert-danger">' . esc_html__( 'Invalid profile ID', 'lifeline-donation' ) . '</div>';
			return;
		}
		/*
	...
	static function process_refund($order) {
		...
		if( $arr->get('post_status') !== 'completed' ) {
			wp_send_json_error( array('message' => esc_html__('Payment was not completed, cannot give refund', 'lifeline-donation') ) );
		}
		if( ! $transaction_id ) {
		...
		*/
		...
		wp_send_json_success( array('message' => esc_html__('Refund is processed successfully in PayPal', 'lifeline-donation') ) );
	}
	...
	static function admin_meta($meta_data, $order) {
		...
		$meta_data[] = [
			...
			'value'	=> ($profile_id) ? esc_html__('YES', 'lifeline-donation') : esc_html__('NO', 'lifeline-donation')
		];
