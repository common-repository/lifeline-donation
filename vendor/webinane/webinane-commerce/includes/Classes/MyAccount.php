...
class MyAccount {
	...
	static function save_profile() {
		...
		if( ! is_user_logged_in() || ! wp_verify_nonce( $nonce, WPCM_GLOBAL_KEY ) ) {
			wp_send_json_error( array('message' => esc_html__( 'You are not authorized', 'lifeline-donation' )) );
		}
		...
		if ( array_get($options, 'user_password') ) {
			if( array_get($_post, 'user_password') < 10 ) {
				wp_send_json_error(['message' => esc_html__('Password length must be at least 10 character', 'lifeline-donation' )], 403);
			}
		}
		...
		if( ! $user ) {
			wp_send_json_error(['message' => esc_html__('No user found', 'lifeline-donation')], 403);
		}
		...
		wp_send_json_success( array('message' => $message. esc_html__( 'Profile Updated', 'lifeline-donation' ), 'data' => $options) );
		return;
		$customer_data = [];
		if( ! isset($user->ID) ) {
			$user = array();
			...
			if(  $confirm_pass = $opt->get('password_confirm') ) {
				if( $password === $confirm_pass ) {
					if( strlen($password) > 5 ) {
						...
						$message = esc_html__('Password length must be at least 10 character', 'lifeline-donation' ) . "\n";
					}
				} else {
					$message = esc_html__('Password and confirm password does not match', 'lifeline-donation' ) . "\n";
				}
			} else {
		...
		wp_send_json_success( array('message' => $message. esc_html__( 'Profile Updated', 'lifeline-donation' ), 'data' => $data) );
	}
	...
	static function login() {
		...
		if( ! wp_verify_nonce( $nonce, WPCM_GLOBAL_KEY ) ) {
			wp_send_json_error( ['message' => esc_html__('Request is not validated.', 'lifeline-donation')] );
		}
		...
		wp_send_json_success( ['message' => esc_html__('Login successful.', 'lifeline-donation')] );
	}
	...
	static function register() {
		...
		if (! get_option( 'users_can_register' ) ) {
			wp_send_json_error( ['message' => esc_html__('Registration is not allowed at the moment', 'lifeline-donation')] );
		...
		if(strlen($password) < 10) {
			wp_send_json_error( ['message' => esc_html__('Minimum password length must be 10.', 'lifeline-donation')] );
		}
		if($password !== $c_password) {
			wp_send_json_error( ['message' => esc_html__('Password and confirm password don\'t match', 'lifeline-donation')] );
		}
		...
		wp_send_json_success( ['message' => esc_html__('Registration successful, please login.', 'lifeline-donation')] );
	}
	...
	private static function register_new_user( $user_login, $user_email, $password ) {
		...
		if ( '' == $sanitized_user_login ) {
			$errors->add( 'empty_username', __( '<strong>Error</strong>: Please enter a username.', 'lifeline-donation' ) );
		} elseif ( ! validate_username( $user_login ) ) {
			$errors->add( 'invalid_username', __( '<strong>Error</strong>: This username is invalid because it uses illegal characters. Please enter a valid username.', 'lifeline-donation' ) );
			$sanitized_user_login = '';
		} elseif ( username_exists( $sanitized_user_login ) ) {
			$errors->add( 'username_exists', __( '<strong>Error</strong>: This username is already registered. Please choose another one.', 'lifeline-donation' ) );

		} else {
			/** This filter is documented in wp-includes/user.php */
			$illegal_user_logins = (array) apply_filters( 'illegal_user_logins', array() );
			if ( in_array( strtolower( $sanitized_user_login ), array_map( 'strtolower', $illegal_user_logins ), true ) ) {
				$errors->add( 'invalid_username', __( '<strong>Error</strong>: Sorry, that username is not allowed.', 'lifeline-donation' ) );
			}
		}

		// Check the email address.
		if ( '' == $user_email ) {
			$errors->add( 'empty_email', __( '<strong>Error</strong>: Please type your email address.', 'lifeline-donation' ) );
		} elseif ( ! is_email( $user_email ) ) {
			$errors->add( 'invalid_email', __( '<strong>Error</strong>: The email address isn&#8217;t correct.', 'lifeline-donation' ) );
			$user_email = '';
		} elseif ( email_exists( $user_email ) ) {
			$errors->add( 'email_exists', __( '<strong>Error</strong>: This email is already registered, please choose another one.', 'lifeline-donation' ) );
		}
		...
		if ( ! $user_id || is_wp_error( $user_id ) ) {
			$errors->add(
				...
				sprintf(
					...
					__( '<strong>Error</strong>: Couldn&#8217;t register you&hellip; please contact the <a href="mailto:%s">webmaster</a> !', 'lifeline-donation' ),
	...
	static function get_user_orders() {
		...
		$user = wp_get_current_user();
		if( ! is_user_logged_in() ) {
			wp_send_json_error( array('message' => esc_html__( 'You are not authorized to access it', 'lifeline-donation' )) );
		}
		...
		$strings = [
			'orders' => apply_filters( 'wpcm_orders_admin_menu_label', esc_html__('Orders', 'lifeline-donation') ),
			'order' => apply_filters( 'wpcm_order_admin_menu_label', esc_html__('Order', 'lifeline-donation') ),
		];
		...
		if( $query->have_posts() ) {
			...
			wp_send_json_error( array('message' => sprintf(esc_html__('You don\'t have any %s', 'lifeline-donation' ), $order_label)) );
		}
	}
	...
	static function get_user_payment_methods() {
		...
		if( $query->have_posts() ) {
			...
			wp_send_json_error( array('message' => esc_html__('You don\'t have any gatways', 'lifeline-donation' )) );
		}
	}
	...
	static function add_order_notes() {
		...
		if( ! $note ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Note should not be empty', 'lifeline-donation' ) ) );
		}

		if( strlen($note) < 10 ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Please write a brief note', 'lifeline-donation' ) ) );
		}
		...
		wp_send_json_success( array( 'notes' => $comments, 'message' => esc_html__('Note is added successfully', 'lifeline-donation' ) ) );
	}
}
