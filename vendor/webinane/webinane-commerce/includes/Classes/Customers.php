...
class Customers
{
	...
	public $meta = array( 'first_name', 'last_name', 'address', 'address_line_1', 'address_line_2', 'city', 'base_country', 'state', 'zip', 'phone', 'company');
	public $customer;
	static $instance;
	...
	function create_new($email, $data) {
		...
		if( is_email( $email ) ) {
			...
			if ( $customer_id ) {
				...
				return new \WP_Error( esc_html__( 'There is something wrong with adding customer', 'lifeline-donation' ) );
			}
		} 
	...
	function insert_meta(Customer $customer, $data = array()) {
		...
		if( $customer ) {
			foreach( array('billing', 'shipping', 'billing_fields') as $address_keys) {
	 ...
	 * 
	 * @return array Returns the complete user info.
	 */
	...
	public function copyBilling(Customer $customer) {
		...
		if( ! $customer ) {
			wp_send_json_error(array('message' => esc_html__('Customer not found', 'lifeline-donation')));
		}
	...
	static function add_new_customer() {
		...
		if( ! is_email($email) ) {
			wp_send_json_error(array('message' => esc_html__('Invalid email address provided', 'lifeline-donation')));
		}
		if( ! $name || strlen($name) < 4 ) {
			wp_send_json_error(array('message' => esc_html__('Name is either empty or size is less than 4', 'lifeline-donation')));
		}
		$customer = new Customer;
		...
		wp_send_json_success( ['message' => esc_html__('Customer is created successfully', 'lifeline-donation'), 'customers' => Customer::all()->toArray()] );
	}
	...
	public static function remove_single_customer() {
		...
		if( ! $customer_id ) {
			wp_send_json_error(array('message' => esc_html__('Invalid Customer ID', 'lifeline-donation')));
		}

		$customer = Customer::find($customer_id);

		if( ! $customer ) {
			wp_send_json_error(array('message' => esc_html__('Given customer not found', 'lifeline-donation')));
		}

		$customer->metas()->delete();
		$customer->delete();

		wp_send_json_success( ['message' => esc_html__('Customer is removed', 'lifeline-donation'), 'customers' => Customer::all()] );
	}
