...
class Checkout {
	...
	static function add_to_cart($args = array() ) {
		...
		return esc_html__( 'Added to cart', 'lifeline-donation' );
	}
	...
	static function remove_from_cart($args = array() ) {
		...
		return esc_html__( 'Removed from cart', 'lifeline-donation' );
	}
	...
	static function process_checkout() {
		...
		if ( ! wp_verify_nonce( $nonce, WPCM_GLOBAL_KEY )) {
			wp_send_json_error( array( 'message' => esc_html__( 'Security verification failed', 'lifeline-donation' ) ) );
		}

		// Make sure the cart isn't empty
		if ( ! wpcm_get_cart_content() ) {
			$valid_data = false;
			wp_send_json_error( array( 'empty_cart' => esc_html__( 'Your cart is empty', 'lifeline-donation' ) ) );
		} else {
		...
		if( ! is_user_logged_in() ) {
			...
			if( ! $email ) {
				wp_send_json_error( esc_html__( 'Please provide a valid email', 'lifeline-donation' ) );
			}
			...
			if ( ! $user_id && false == email_exists( $email ) ) {
				...
				if( $user_id ) {
					...
					wp_send_json_error( esc_html__('Something went wrong', 'lifeline-donation' ) );
				}
			}
		}
		...
		if( ! isset( $user->ID ) ) {
			wp_send_json_error( esc_html__( 'Please refresh the page and try again', 'lifeline-donation' ) );
		}
	...
	static function validate_fields($data, $is_ajax) {
		...
		$validation->setAliases([
			'billing.email' => esc_html__('Billing Email', 'lifeline-donation'),
			'billing.first_name' => esc_html__('Billing First Name', 'lifeline-donation'),
			'billing.address_line_1' => esc_html__('Billing Address', 'lifeline-donation'),
			'billing.city' => esc_html__('Billing City', 'lifeline-donation'),
			'billing.base_country' => esc_html__('Billing Country', 'lifeline-donation'),
			'nonce' => esc_html__('Security check', 'lifeline-donation'),
			'gateway' => esc_html__('Payment Gateway', 'lifeline-donation'),
		]);
