...
class ExportOrders
{
	...
	static function user_export_opt(){
		...
		return wp_send_json_error(esc_html__('Please select the columns and post status.', 'lifeline-donation'));
	}
	...
	static function export($columns, $status, $st_date='', $nd_date=''){
		...
		wp_reset_postdata();
		if ( empty( $data_array ) ) {
				wp_send_json_error( ['message' => esc_html__( 'No record found', 'lifeline-donation' )] );
		}
