...
class Ajax 
{
	...
	static function ajax_add_to_cart() {
		...
		if ( ! wp_verify_nonce( $nonce, WPCM_GLOBAL_KEY )) {
			wp_send_json( array('success' => false, 'message' => esc_html__( 'Security verification failed', 'lifeline-donation' ) ), 403 );
		}
		...
		if ( ! $id ) {
			wp_send_json( array('success' => false, 'message' => esc_html__( 'Item id is not provided', 'lifeline-donation' ) ), 403 );
		}
		...
		if( wpcm_get_settings()->get('redirect_to_checkout') ) {
			...
			if( $url ) {
				wp_send_json( array('success' => true, 'message' => esc_html__( 'Item added to cart', 'lifeline-donation' ), 'redirect' => esc_url($url) ), 200 );
			}
		}
		wp_send_json( array('success' => true, 'message' => esc_html__( 'Item added to cart', 'lifeline-donation' ) ), 200 );
	...
	static function checkout() {
		...
		$nonce = esc_attr( webinane_set($_POST, 'nonce' ) );
		if( ! wp_verify_nonce( $nonce, WPCM_GLOBAL_KEY ) ) {
			wp_send_json( array('success' => false, 'message' => esc_html__( 'Security check failed, try again after refresh the page', 403, 'webinane-commerce', 'lifeline-donation' )) );
		}
