...
class Emails extends Email
{
	...
	static function send_customer_notif($order, $customer_id) {
		...
		if( $customer ) {
			...
			if( is_email( $email ) ) {
				$subject = sprintf( esc_html__( 'You order has been received - %s', 'lifeline-donation' ), get_bloginfo( 'name' ) );
	...
	static function send_owner_notif($order, $customer_id) {
		...
		if( is_email( $email ) ) {
			$settings = wpcm_get_settings();
			$subject = sprintf( esc_html__( 'New order is received - %s', 'lifeline-donation' ), get_bloginfo( 'name' ) );
	...
	static function email_invoice($order, $customer_id) {
		...
		if( is_email( $customer_data->email ) ) {
			...
			$subject = apply_filters( 'webinane_commerce_customer_invoice_email_subject', sprintf( esc_html__( 'Invoice for order # %s - %s', 'lifeline-donation' ), $order['ID'], get_bloginfo('name') ), $order );

			wp_mail( $customer_data->email, $subject, $template, $headers, [$attachment] );
	...
	static function generate_invoice($odr, $customer_data) {
		...
		$order_data = WebinaneCommerce\Orders::order_data($order);
		...
		$invoice->addTotal(esc_html__("Total", 'lifeline-donation' ),$total);
		$invoice->addTotal(esc_html__("VAT", 'lifeline-donation' ), 0);
		$invoice->addTotal(esc_html__("Total due", 'lifeline-donation' ),$total,true);
		...
		$invoice->addTitle(esc_html__("Important Notice", 'lifeline-donation' ));
	...
	/**
	 * Email Body.
	 * @param  [type] $placeholders [description]
	 * @return [type]               [description]
	 */
	...
	private static function placeholders($order, $customer) {
		...
		$price_label = apply_filters('wpcm_price_label', esc_html__('Price', 'lifeline-donation'));
		$placeholders = [
