...
class Api
{
    ...
    private static $api_server='https://www.webinane.com';
    ...
    /*public function update() {
        ...
        update_option('wpcm_user_data', $storage);
    }*/
    ...
    public static function is_connected() {
        $options = get_option('webinane_commerce_user_connect');
    ...
    private static function is_expired($info) {
        ...
        if ( isset( $info['expires_in'] ) ) {
            return $info['expires_in'] > time() ? false : true;
        }
    ...
    public static function is_disconnected() {
        ...
        if(!$opt_disconnect){
            wp_send_json_error( ['message' => esc_html__('settings to disconnection is get wrong.', 'lifeline-donation')], 403 );
        }
    ...
    public static function parse_response($response) {
        ...
        if($status == 200) {
            ...
            update_option(self::OPT_KEY, $body);
            set_transient( self::TRANSIENT, $body, DAY_IN_SECONDS * 100 );

            return ['message' => esc_html__('Connection is successful', 'lifeline-donation')];

        } else {
            $body = wp_remote_retrieve_body( $response );
            if(json_decode($body)) {
                // $body = json_decode($body);
                return new WP_Error('failed', esc_html__( 'Wrong username or password, please try again with correct credentials', 'lifeline-donation' ));
            } else {
