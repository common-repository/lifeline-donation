...
class Connect
{
	...
	static function validate_request() {
		...
		if(! wp_verify_nonce( $nonce, WPCM_AJAX_ACTION )) {
			wp_send_json_error( ['message' => esc_html__('Request is not verified', 'lifeline-donation')], 403 );
		}
		if(! current_user_can( 'manage_options' )) {
			wp_send_json_error( ['message' => esc_html__('You are not authorized to that', 'lifeline-donation')], 403 );
		}
	}
	...
	static function disconnect_user() {
		...
		if($status == 200 || $status == 401) {
			...
			if(!$is_disconnected){
				wp_send_json_error( ['message' => esc_html__('trash to disconnect is get wrong.', 'lifeline-donation')], 403 );
			}
		...
		wp_send_json_error( ['message' => esc_html__('Unable to disconnected from the server, try again later', 'lifeline-donation')], 403 );
	...
	static function get_items() {
		...
		if ( ! $items ) {
			...
			wp_send_json_error( ['message' => esc_html__('Unable to connect the API, try again later', 'lifeline-donation')], 403 );
		} else {
	...
	static function get_user_items() {
		...
		return new WP_Error( 'message', esc_html__('Unable to connect the API, try again later', 'lifeline-donation') );
	}
	...
	public static function install_plugin() {
		...
		if($status == 200) {
			...
			wp_send_json_success( ['message' => esc_html__('Plugin is installed and activated successfully', 'lifeline-donation')] );
	...
	public static function activate_plugin() {
		...
		wp_send_json_success( ['message' => esc_html__( 'Plugin is activated successfully', 'lifeline-donation' )] );
	}
}
