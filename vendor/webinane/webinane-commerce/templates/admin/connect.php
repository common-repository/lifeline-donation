...
?>
<h3 class="hndle" style="display: none;"><?php esc_html_e('Connect', 'lifeline-donation') ?></h3>
...
<div id="wpcm-admin-live-connect">
		  ...
		  :is_active="true">
			<div v-show="loaded" style="display: none; max-width: 95%;">
				<div class="extensions-head" style="width: 100%;display: block;position: relative;">
					<h3 style="margin-bottom: 30px;"><?php esc_html_e('Our Extensions', 'lifeline-donation') ?></h3>
					<el-button @click="login_out()" :loading="loading" style="position: absolute; top: -8px; left: 140px;" type="danger"><?php esc_html_e('Disconnect', 'lifeline-donation'); ?></el-button>
				</div>
					      ...
					      <div style="padding: 14px;">
					        ...
					        <div class="bottom clearfix">
					          ...
					          <a v-if="is_purchased(item.wp_slug) && !is_installed(item.wp_slug) " :href="item.url" target="_blank" class="button " @click.prevent="installPlugin(item)">
					          	<?php esc_html_e('Install', 'lifeline-donation') ?>
					          </a>
	...
	<?php else:  ?>
		<connect inline-template nonce="<?php echo wp_create_nonce( WPCM_AJAX_ACTION ) ?>" :is_active="false">
			<el-row>
				<el-col :span="10">
					<el-card header="<?php esc_html_e('Login in to Connect', 'lifeline-donation') ?>" v-loading="loading">
						<el-form :model="form">
							<el-form-item>
								<el-input v-model="form.email" placeholder="<?php esc_html_e('Email', 'lifeline-donation') ?>"></el-input>
							</el-form-item>
							<el-form-item>
								<el-input placeholder="<?php esc_html_e('Password', 'lifeline-donation') ?>" type="password" v-model="form.password"></el-input>
							</el-form-item>
							<el-row :gutter="30">
								<el-col :span="12">
									<?php printf( __('Don\'t have account? <a href="%s" target="_blank">Create one</a>', 'lifeline-donation'), 'https://www.webinane.com/register') ?>
								</el-col>
								<el-col :span="12" :style="{textAlign: 'right'}">
									<el-button @click="login()" type="primary"><?php esc_html_e('Login', 'lifeline-donation') ?></el-button>
								</el-col>
							</el-row>
						</el-form>
					</el-card>
